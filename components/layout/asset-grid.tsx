                ...
                <div className="relative aspect-square bg-zinc-100">
                  {showImage ? (
                    <Image
                      ...
                      fill
                      onError={() => {
                        setBrokenImageIds((current) => new Set(current).add(item.id));
                      }}
                    ...
                    <div className="flex h-full flex-col items-center justify-center gap-1 px-3 text-center text-xs font-medium text-zinc-500">
                      {imageBroken ? (
                        <>
                          <span>链接已失效</span>
                          <span className="text-[10px] text-zinc-400">请删除后重新生成</span>
                        </>
                      ) : (
                        item.mimeType ?? "文件"
                      )}
      ...
      {previewItem ? (
        ...
        >
          <div className="flex shrink-0 items-center justify-between gap-3 pb-3">
            <p className="truncate text-sm font-medium text-white" title={previewItem.name}>
              {previewItem.name}
            </p>
            <button
              aria-label="关闭预览"
