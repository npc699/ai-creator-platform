...
type FeedListItemSkeletonProps = {
  variant?: "default" | "hot" | "viral";
  withCover?: boolean;
  withAvatar?: boolean;
};

/** 全站 Feed 列表行骨架，对齐 FeedListItem 单元格布局。 */
export function FeedListItemSkeleton({
  variant = "default",
  withCover = false,
  withAvatar = true,
}: FeedListItemSkeletonProps) {
  return (
...
/** @deprecated 使用 FeedListItemSkeleton variant="default" */
export function FeedArticleCardSkeleton({
  withCover = false,
}: {
  withCover?: boolean;
}) {
  return <FeedListItemSkeleton variant="default" withCover={withCover} />;
}

/** @deprecated 使用 FeedListItemSkeleton variant="hot" */
export function ChannelFeedListItemSkeleton({
  variant,
  withCover = false,
}: {
  variant: "hot" | "viral";
  ...
  return <FeedListItemSkeleton variant={variant} withCover={withCover} withAvatar={false} />;
}

/** @deprecated 使用 FeedListItemSkeleton variant="hot" */
export function HotFeedListItemSkeleton() {
  return <FeedListItemSkeleton variant="hot" withAvatar={false} />;
}

/** @deprecated 使用 FeedListItemSkeleton variant="viral" */
export function ViralFeedListItemSkeleton() {
  return <FeedListItemSkeleton variant="viral" withAvatar={false} />;
}
