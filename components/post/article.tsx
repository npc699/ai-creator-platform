...
import { ArticleMetaBadges } from "@/components/feed";
import { AuthorAvatar } from "@/components/author";
import { useFeedLikeToggle } from "@/components/feed";
...
function PostReviewPendingBanner({ postId }: { postId: string }) {
  const router = useRouter();
...
/** 文章详情主体：发布 Toast、标题/作者/指标、审核横幅。 */
export function PostReaderArticle({
  title,
  authorId,
  authorName,
  authorImage,
  ...
  return (
    <>
      ...
      {showReviewBanner ? <PostReviewPendingBanner postId={postId} /> : null}
    </>
  );
}
